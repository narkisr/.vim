...
"
" 1.1 2011-06-15
" ri-viewer.vim to plugin dir:
"   Reported by Volodymyr.
"
" 1.2 2011-06-16
"
" ColorHighlight integrated with AnsiEsc:
"   Recommended by Volodymyr. Thanks for your help!
...
command! -bang -nargs=0 RiEscape call <sid>RiAnsiEsc(<bang>0)
...
func! s:RiOptions(target)
  ...
  if l:ri_version =~ '1\.[0-9]\.[0-9]'
    ...
    " old ri
    if a:target == 'core'
      let l:options = '-f ansi -T --system --site'
    elseif a:target == 'gem'
      let l:options = '-f ansi -T --gems'
    elseif a:target == 'home'
      let l:options = '-f ansi -T --home'
    elseif a:target == 'list'
      let l:options = '-f ansi -T --list-names'
    elseif a:target == 'dirs'
      let l:options = '--help'
    else
      let l:options = '-f ansi -T --system --site --gems --home'
    endif
  else
    " standard ri included in ruby 1.9.x
    if a:target == 'core'
      let l:options = '-f ansi -T --system --site --no-gems --no-home'
    elseif a:target == 'gem'
      let l:options = '-f ansi -T --no-system --no-site --gems --no-home'
    elseif a:target == 'home'
      let l:options = '-f ansi -T --no-system --no-site --no-gems --home'
    elseif a:target == 'list'
      let l:options = ''
    elseif a:target == 'dirs'
      let l:options = '--list-doc-dirs'
    else
      let l:options = '-f ansi -T'
    endif
  endif
...
" Originally from AnsiESC.vim by Charles Campbell
" toggles ansi-escape code visualization
func! <sid>RiAnsiEsc(rebuild)
...
" Originally from AnsiESC.vim by Charles Campbell
" builds custom syntax highlighting for three or more element ansi escape sequences
